	...
	if(freeSpace != -1)
	{
		for(j = 0 ; j < 2 ; j++)//if all pages happen to be ref=1, do another loop and evict ref=0
		{
			for(i = 0 ; i < 1000 ; i++)
			{
				...
				{
					int a = ram[page_table[i].physical_address - 0] ;
					ssd[freeSpace] = a ;
	...
	else if(freeSpace != -1)
	{
		...
		for(i = 0 ; i < 1000 ; i++)
		{
			if(page_table[i].valid == 0)// && page_table[i].physical_address == -1)//
			{
				...
				page_table[i].physical_address = freeSpace + 0;
				page_table[i].lock = 0 ;
...
{// Aguments explanation:
 ...
 // memory hierarchy level "memory"
		...
		if(freeSpace != -1)
		{
		  ...
		  mem[memory][freeSpace] = mem[below(memory)][page_table[address].physical_address-mem_offset[memory]] ;
		  //ram[page_table[i].physical_address] = -1 ;
		...
		{//evict something from ram
			...
			//swap values
				...
				mem[below(memory)][physical_address-below(memory)] = mem[memory][page_table[evict_from].physical_address - mem_offset[memory]];//
...
void pageFaultHandler_ram(vAddr address,int memory)
{// Aguments explanation:
 // we want to swap the page with virtal address "address" to the
 // memory hierarchy level "memory"
		int freeSpace = findFREE(memory) ;
