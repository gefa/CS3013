...
{// Aguments explanation:
 ...
 // memory hierarchy level "memory"
		...
		{//evict something from ram
			...
			if(findFREE(below(memory))==-1)// do we have to evict?
			{	// yes we no
...
void count(int memory)
{
  ...
  for(i=0;i<HD_SIZE;++i)
    {
      if(page_table[i].memory_location == memory && page_table[i].valid == 1)
	cnt++;

      if(page_table[i].valid == 1)
	page_count++;						
    }
  printf("Memory[%d]: %d, TOTAL: %d\n",memory,cnt,page_count);
